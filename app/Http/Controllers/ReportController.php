...
class ReportController extends Controller
{
    ...
    /**
     * Shows M-Pesa transactions report
     *
     * @return \Illuminate\Http\Response
     */
    ...
    {
        if ($request->ajax()) {
            $query = \Parcy\Mpesa\Models\MpesaTransaction::query()
                ->select([
                    'id',
                    'type',
                    'status',
            ...
            if (!empty($request->input('date_range'))) {
                ...
                if (count($date_range_array) == 2) {
                    $query->whereDate('created_at', '>=', trim($date_range_array[0]))
                        ->whereDate('created_at', '<=', trim($date_range_array[1]));
                }
            }
            ...
            return Datatables::of($query)
                ...
                ->editColumn('status', function ($row) {
                    ...
                    $class = $badges[$row->status] ?? 'bg-blue';
        ...
        return view('report.mpesa_transactions');
    }
}
